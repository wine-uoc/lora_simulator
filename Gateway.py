...
class Gateway:
    def __init__(self, uid):
        ...
        # If this is set to False, the gateway does not return a DR according to the distance
        self.allow_update_dr = None
    ...
    def place_mid(self, sim_map):
        ...
        self.x = self.sim_map.size_x / 2.0
        self.y = self.sim_map.size_y / 2.0
    ...
    def set_update_dr(self, state):
        self.allow_update_dr = state
    ...
    def set_sf_thresholds(self, mode, th=None):
        ...
        if mode == "equal" and self.sim_map:
            ...
            self.th = tuple(
                np.linspace(start=0, stop=self.sim_map.size_x, num=6, endpoint=False)[
                    1:
                ]
            )
    ...
    def get_data_rate(self, target):
        """
        Return a DR according to distance relative to target if allowed
        """
        if self.allow_update_dr:
            # Get distance
            dist = PositionHelper.PositionHelper.get_distance(
                node_a=self, node_b=target
            )
