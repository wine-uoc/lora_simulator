...
class Device:
    ...
    # tx_interval:   The period that the node will transmit in seconds (milliseconds)
    ...
    def __init__(
        ...
        # NOTE: frame_list[i] NOT always returns packet i, as packet number can be repeated if FHSS split
        self.frame_list = []
        ...
        if (gateway != None and dr < 6):
            # If a gateway exists and is a LoRa device
            self.dr = gateway.return_data_rate(target=self)
            
        logger.info(f"Created {self.modulation} node with id={self.device_id}, position x={self.pos_x}, y={self.pos_y} and DR={self.dr}.")
    ...
    def create_frame(self, current_time, duration):
        frame = Frame.Frame(owner      = self.device_id,
