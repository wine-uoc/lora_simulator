...
class Device:
    ...
    def __init__(self, device_id=None, time_mode=None, tx_interval=None, tx_rate=None, tx_payload=None, modulation=None):
        ...
        # The list of packets transmitted by the node
        self.pkt_list = []
    ...
    def create_packet(self, current_time):
        packet = Packet.Frame(self.device_id, self.get_num_packet(), self.tx_duration_ms, self.modulation, current_time)
    ...
    def init(self):
        ...
        self.next_time = TimeHelper.TimeHelper.next_time(current_time=0, step_time=self.tx_interval, mode=self.time_mode, tx_duration=self.tx_duration_ms)
    ...
    def time_step(self, current_time=None, maximum_time=None, sim_grid=None):
        ...
        if current_time == self.next_time:
            ...
            # Create packet
            packet = self.create_packet(current_time)

            # Transmit
            Transmission.transmit(packet, sim_grid)

            # Generate a time for the next transmission
            next_time = TimeHelper.TimeHelper.next_time(current_time=current_time, step_time=self.tx_interval, mode=self.time_mode, tx_duration=self.tx_duration_ms)
