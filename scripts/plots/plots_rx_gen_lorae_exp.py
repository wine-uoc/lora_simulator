...
cmap = matplotlib.colors.LinearSegmentedColormap.from_list('Custom cmap', cmaplist, cmap.N)     # create the temp map
...
for i in range(len(dvs)):
    ...
    plt.plot(np.array(gen_dr0) * dvs[i] * 10,
             np.array(rxed_dr0) * dvs[i] * 10 * 4/5,
             linewidth=3.0, c=colors[i], linestyle=':')  # , label=r'DR0 N=' + str(dvs[i])
# dvs2 = [900, 100, 0]
...
for i in range(len(dvs)):
    ...
    plt.plot(np.array(gen_dr8) * dvs[i] * 10,
             np.array(rxed_dr8) * dvs[i] * 10 * 1/3,
             linewidth=3.0, c=colors[i], linestyle='-')
# for i in range(len(dvs)):
...
#
# plt.axvline(x=1000*max(lmbd_dr0)*10, color='blue', linestyle=':')
# plt.axvline(x=100*max(lmbd_dr0)*10, color='blue', linestyle=':')
# plt.axvline(x=10*max(lmbd_dr0)*10, color='blue', linestyle=':')
# plt.axvline(x=1000*max(lmbd_dr8)*10, color='red', linestyle=':')
# plt.axvline(x=100*max(lmbd_dr8)*10, color='red', linestyle=':')
# plt.axvline(x=10*max(lmbd_dr8)*10, color='red', linestyle=':')
