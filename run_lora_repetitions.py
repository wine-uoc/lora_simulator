...
def get_t_off(_dr, _pl):
    ...
    return LoraHelper.LoraHelper.get_off_period(toa, 0.01), toa


python = "python3"
script = "Simulator.py"

result_file = './results/dr{}/pl{}/{}_{}_{}_{}'
result_ext  = ".npy"
...
payloads  = [50]
devices   = [5,10,15,20,25,40,50,70,100,150,200]
lmbd         = np.arange(1, 20, 1)    # 1 pkt/h/node until 20, will be limited by 0.01 DC for each data rate
tx_intervals = list(np.round(1. / lmbd * 3600000).astype(int))  # Conversion to tx interval in simulation units (ms)
frame_repetitions = [1,2,3,4]
        ...
        for device in devices:
            ...
            if device > 1000 and datarate < 8:
                ...
                for interval in tx_intervals:
                    ...
                    for repetitions in frame_repetitions:
                        # Repeat for number of runs
                        for i in range(runs):
                            data_file = result_file.format(datarate, payload, device, interval, repetitions, i) + result_ext
                            log_file = result_file.format(datarate, payload, device, interval, repetitions, i)
                            ...
                            # Execute the simulation by calling the Simulator.py with the appropriate parameters
                                ...
                                command = "{} {} -r {} -d {} -t {} -pl {} -dr {} -re {} -l {}".format(python, script, i, device,
                                                                                            interval, payload, datarate, repetitions,
                                                                                            log_file)
                                os.system(command)
